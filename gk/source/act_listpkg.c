...
{
    printf("usage:%s -c conf -k listpkg -u [me/any] -r [repo]\n", argv[0]);
}

int act_listpkg_init(int argc, char* argv[], gk_conf_t * gc)
{
	int opt;
	struct stat st;
	//char action[GK_STR_LEN] = {0};
    	char *string = "hc:k:u:r:";
	optind = 1;
	//opt='\0';
	...
	if(strlen(gc->repo) == 0) //{printf("arch is needed\n"); my_usage(argc, argv);return 1;}
	{
		sprintf(gc->repo, "%s", "any");
	}
	...
	#ifdef _DEBUG_
	printf("%s:%d:parese args success,conf=%s,user=%s,reo=%s\n", __FILE__, __LINE__, gc->conf, gc->user, gc->repo);
	#endif
	return 0;
}


int list_pkg(gk_conf_t * gc)
{
	CURL *curl;
    //int still_running;
    ...
    sprintf(strurl, "http://%s/gk_listpkg.php?", gc->url);
	#ifdef _DEBUG_
    ...
    if (curl)
    {
		CURLcode res;
		char data[256] = {0};
		sprintf(data, "uid=%s&token=%s&out=%s&user=%s&repo=%s", gc->uid, strts, gc->output, gc->user, gc->repo);
